...
# Use a VBScript shim so the process is launched with SW_HIDE at the
# CreateProcess level. This prevents the brief pwsh.exe window flash that
# occurs even with -WindowStyle Hidden when Task Scheduler is the launcher.
$keepAliveVbs = Join-Path $PSScriptRoot "KeepAlive.vbs"

if (-not (Test-Path $keepAliveVbs)) {
    Write-Host "[!] KeepAlive.vbs not found at: $keepAliveVbs" -ForegroundColor Red
    exit 1
}

$action = New-ScheduledTaskAction `
    -Execute  "wscript.exe" `
    -Argument "`"$keepAliveVbs`""
...
Write-Host "    VBS shim  : $keepAliveVbs" -ForegroundColor Gray
