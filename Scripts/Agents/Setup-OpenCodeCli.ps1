...
<#
.SYNOPSIS
    Symlink ~/.config/opencode/AGENTS.md to a personal preferences file.

.DESCRIPTION
    Creates a single file symlink from ~/.config/opencode/AGENTS.md to the
    file at -PreferencesFile. Existing non-symlink content is moved aside
    to <linkPath>.backup.<timestamp> first (skipped under -Force).
    Re-launches elevated if the OS rejects the symlink for lack of
    privilege.
...
$linkPath        = Join-Path $HOME '.config' 'opencode' 'AGENTS.md'
...
Write-Host ''
Write-Host 'OpenCode preferences' -ForegroundColor Cyan
